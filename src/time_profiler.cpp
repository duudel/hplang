    ...
    : event_index(next_event++)
{
    ...
    event_store[event_index].end_event_index = -1; // is a begin
}
...
Timed_Scope::~Timed_Scope()
{
    ...
    event_store[end_event_index].end_event_index = event_index;
}
...
void CollateProfilingData(Compiler_Context *ctx)
{
    ...
    for (s64 i = 0; i < event_count; i++)
    {
        ...
        {
            Profiling_Event begin_ev = event_store[event.end_event_index];
